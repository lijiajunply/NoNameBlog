...
const mdxComponents: Record<string, any> = {
  ...
  ul: ({ className, children, ...props }: MdxComponentProps) => (
    <ul className={cn("my-6 ml-6 list-disc space-y-3 marker:text-neutral-400 dark:marker:text-neutral-600", className)} {...props}>{children}</ul>
  ),
  ol: ({ className, children, ...props }: MdxComponentProps) => (
    <ol
      className={cn("my-6 ml-6 list-decimal space-y-3 marker:text-neutral-500 dark:marker:text-neutral-500", className)}
      {...props}
    >{children}</ol>
  ),
  li: ({ className, children, ...props }: MdxComponentProps) => (
    <li
      className={cn("pl-2 text-neutral-700 dark:text-neutral-300 leading-relaxed", className)}
      {...props}
    >{children}</li>
  ),
  ...
  thead: ({ className, children, ...props }: MdxComponentProps) => (
    <thead className={cn("bg-neutral-50/80 dark:bg-neutral-800/80 backdrop-blur-md", className)} {...props}>{children}</thead>
  ),
  ...
  img: ({ className, alt, ...props }: MdxComponentProps) => (
    ...
    <img
      className={cn("rounded-2xl shadow-sm my-8 border border-neutral-200/50 dark:border-neutral-800/50", className)}
      alt={alt}
      {...props}
    />
  ),
  ...
  pre: ({ className, children, ...props }: MdxComponentProps) => (
    <pre
      className={cn(
        "mb-6 mt-6 overflow-x-auto rounded-xl py-4",
        className
      )}
      {...props}
    >{children}</pre>

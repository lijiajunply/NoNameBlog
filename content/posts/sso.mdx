...
sequenceDiagram
    ...
    rect rgb(230, 240, 255)
        Note over U, SSO: 第一步：发起授权请求
        U ->> App: 点击「SSO 登录」
        App ->> SSO: GET /authorize<br/>?response_type=code<br/>&client_id=CLIENT_ID<br/>&redirect_uri=REDIRECT_URI<br/>&scope=SCOPE<br/>&state=STATE
    end

    rect rgb(255, 243, 224)
        Note over U, SSO: 第二步：用户登录 & 授权，生成授权码
        SSO -->> U: 重定向到 SSO 登录页面
        U ->> SSO: 输入用户名 / 密码，完成身份验证
        U ->> SSO: 同意授权
        SSO ->> SSO: ✅ 验证通过，生成临时授权码（Authorization Code）
    end

    rect rgb(232, 245, 233)
        Note over U, SSO: 第三步：携带授权码回调第三方应用
        SSO -->> U: 302 重定向到 redirect_uri<br/>?code=AUTH_CODE&state=STATE
        U ->> App: 浏览器跟随重定向，携带 code + state
    end

    rect rgb(243, 229, 245)
        Note over U, SSO: 第四步：用授权码换取令牌（后端 → 后端）
        App ->> SSO: POST /token<br/>grant_type=authorization_code<br/>&code=AUTH_CODE<br/>&client_id=CLIENT_ID<br/>&client_secret=CLIENT_SECRET<br/>&redirect_uri=REDIRECT_URI
        SSO ->> SSO: 校验 code、client_id、client_secret
        SSO -->> App: 返回 access_token（+ refresh_token 等）
    end

    rect rgb(255, 235, 238)
        Note over U, SSO: 第五步：使用令牌获取用户信息
        App ->> SSO: GET /userinfo<br/>Authorization: Bearer ACCESS_TOKEN<br/>（按 scope 返回对应字段）
        SSO -->> App: 返回用户数据（如 sub, name, email …）
    end

...
> 这也是个我不会的图表语法，支持 UML 各种图表：

```plantuml
@startuml
Alice -> Bob: 你好，Bob！
Bob --> Alice: 你好，Alice！很高兴见到你。
Alice -> Bob: 你最近怎么样？
```

### Infographic

infographic 是 [Antv](https://infographic.antv.vision/) 开发的一个信息图语法，支持多种图表类型。

> 从创意上讲，跟我后面自制组件有异曲同工之妙。说实话我也是前几天翻网站发现的这个东西。说实话要是单看图片基础的话我那个也是 ok 的，但是这个用来写 PPT 很爽了。我打算基于这个和之前看到的 Markdown to PPT 项目，结合一下
...
目前已经接入的是 `badge` 和 `tip`，所以也可以把它们放进一句普通文字里使用，比如：这个功能现在属于 :badge[实验性](round) 状态，而这里有个 :tip[小提示](悬停可见)。
...
> 我发现这个世界地图上，藏南地区是“给“了印度的，所以我在这里就不贴了
